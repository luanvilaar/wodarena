...
export async function sendQualifierResubmissionRequestedEmail(params: {
  to: string;
  athleteName: string;
  eventName: string;
  workoutName: string;
  ...
  const apiKey = getResendApiKey();
  if (!apiKey) {
    console.error('[Resend Qualifier Resubmission] RESENDAPI_KEY não configurada no ambiente.');
    return { success: false, error: 'Chave de API do Resend ausente.' };
  }
  ...
  const safeEventName = escapeHtml(params.eventName || 'Evento WODArena');
  const safeWorkoutName = escapeHtml(params.workoutName || 'Prova');
  const safeJustification = escapeHtml(params.justification || '');
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Novo envio solicitado - WODArena</title>
      <style>
        body { margin: 0; padding: 0; background: #f5f5f5; color: #181a20; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; }
        .wrapper { width: 100%; padding: 32px 0; background: #f5f5f5; }
        .container { max-width: 560px; margin: 0 auto; background: #ffffff; border: 1px solid #eaecef; border-radius: 12px; overflow: hidden; }
    ...
    <body>
      <div class="wrapper">
        <div class="container">
          <div class="header"><div class="brand">WODArena</div></div>
          <div class="body">
            <span class="eyebrow">Qualifier online</span>
            <h1>Novo envio solicitado</h1>
            <p>Olá, <strong>${safeAthleteName}</strong>. A organização do evento <strong>${safeEventName}</strong> removeu o resultado que você enviou para a prova abaixo e liberou um novo envio.</p>
            <table class="info">
              <tr><td class="label">Prova</td><td class="value">${safeWorkoutName}</td></tr>
              <tr><td class="label">Situação</td><td class="value">Aguardando seu reenvio</td></tr>
            </table>
            <div class="notice">
              Acesse sua área do atleta na WODArena e envie um novo vídeo e resultado para esta prova antes do prazo final de envio.
              ${safeJustification ? `<br><br><strong>Motivo informado pela organização:</strong><br>${safeJustification}` : ''}
            </div>
          </div>
          <div class="footer">
            Esta mensagem foi enviada automaticamente pela WODArena para avisar sobre uma alteração no seu resultado do qualifier.
          </div>
        </div>
      </div>
    </body>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: getResendFrom(),
        to: params.to,
        subject: `Novo envio solicitado - ${params.eventName || 'Evento WODArena'}`,
        html: htmlContent,
      }),
    });
    ...
    if (!res.ok) {
      const errorData = await parseResendError(res);
      console.error('[Resend Qualifier Resubmission] Erro na API do Resend:', errorData);
      return { success: false, error: errorData };
    }
    ...
    console.error('[Resend Qualifier Resubmission] Erro crítico ao enviar e-mail:', err);
    return { success: false, error: err };
  }
}
