  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={submit} className="space-y-4 rounded-xl border border-card-border bg-card p-5 text-white">
        ...
        {existing && <p className="text-xs text-muted">Status atual: <strong className="text-white">{existing.status}</strong> · versão {existing.currentVersion}. {existing.status !== 'pending_review' ? 'Para nova análise após decisão, use a contestação.' : ''}</p>}
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <input required disabled={windowState !== 'open' || existing?.status !== undefined && existing.status !== 'pending_review'} value={result} onChange={e => setResult(e.target.value)} placeholder={workout?.type === 'fortime' ? 'Score (MM:SS)' : 'Score numérico'} className="rounded-md border border-card-border bg-dark-gray px-3 py-2 text-sm text-white disabled:opacity-50" />
          <input required disabled={windowState !== 'open' || existing?.status !== undefined && existing.status !== 'pending_review'} value={videoUrl} onChange={e => setVideoUrl(e.target.value)} placeholder="Link do vídeo no YouTube" className="rounded-md border border-card-border bg-dark-gray px-3 py-2 text-sm text-white disabled:opacity-50" />
        </div>
        <textarea disabled={windowState !== 'open' || existing?.status !== undefined && existing.status !== 'pending_review'} value={athleteNote} onChange={e => setAthleteNote(e.target.value)} rows={3} placeholder="Observação opcional para a arbitragem" className="w-full rounded-md border border-card-border bg-dark-gray px-3 py-2 text-sm text-white disabled:opacity-50" />
        <button disabled={saving || windowState !== 'open' || existing?.status !== undefined && existing.status !== 'pending_review'} className="rounded-md bg-primary px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-ink disabled:opacity-50">{saving ? 'Enviando...' : existing ? 'Atualizar submissão' : 'Enviar resultado'}</button>
