...
- Variáveis de ambiente do app (distintas das chaves do AIOX em `.env.example`): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `WODA_SESSION_SECRET`, `MERCADOPAGO_CLIENT_ID/CLIENT_SECRET/REDIRECT_URI/WEBHOOK_SECRET`, `MERCADOPAGO_PLATFORM_USER_ID` (id da conta Mercado Pago dona da aplicação marketplace — o MP recusa `application_fee` quando o vendedor é essa própria conta, então ela é bloqueada como conta de gestor), `STRIPE_SECRET_KEY`/`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`/`STRIPE_WEBHOOK_SECRET` (gateway usado para eventos em EUR/GBP via Stripe Connect — Mercado Pago continua exclusivo de BRL; ver `src/lib/stripeServer.ts` e `src/lib/paymentGateway.ts`), `RESEND_API_KEY`/`RESEND_FROM_EMAIL`, `WODARENA_OWNER_EMAIL`, `COMMERCIAL_LEADS_OWNER_EMAIL`, `APP_URL`/`NEXT_PUBLIC_APP_URL`.
